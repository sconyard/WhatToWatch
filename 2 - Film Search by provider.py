...
st.info('Want to limit your search to a specific watch provider? Select a watch provider and a genre and we will find the top rated films to watch.')

api_key = '**enter your api key here**'

wprovider_api = f'https://api.themoviedb.org/3/watch/providers/movie?api_key={api_key}&language=en-US&watch_region=GB'
search_wproviders = requests.get(wprovider_api).json()['results']
search_wproviders_json_df = json_normalize(search_wproviders)
search_wprovider_df = pd.DataFrame(search_wproviders_json_df, columns=['provider_name', 'provider_id'])

wprovider_selection = st.selectbox('Select a watch provider', (search_wprovider_df["provider_name"].unique()))

selected_wprovider_code = search_wprovider_df.loc[search_wprovider_df.provider_name == wprovider_selection]["provider_id"].iloc[0]
st.write(f'Watch Provider: {selected_wprovider_code}')
...
filmshow_api = f'https://api.themoviedb.org/3/discover/movie?api_key={api_key}&language=en-US&sort_by=popularity.desc&with_watch_providers={selected_wprovider_code}&watch_region=GB&with_genres={selected_genre_code}'
